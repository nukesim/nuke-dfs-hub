...
with c2:
    with st.container(border=True):
        ...
        st.caption("DraftKings & FanDuel: **No players vs opposing defense** is enabled by default. When checked, a lineup containing a defense cannot contain any QB/RB/WR/TE from that defense's opponent. Turn it off only if you intentionally want offense-vs-defense combinations, then rerun NUKE Sim.")
...
st.info("**FanDuel AnyFLEX Lock:** Use **Lock AnyFLEX** when you want a specific player — such as an RB — included in every generated FanDuel lineup as an AnyFLEX while preventing that player from being used at MVP. You may lock multiple players, up to the five available AnyFLEX positions. Rerun the Showdown SIM after changing locks so the candidate pool is regenerated with those requirements.")
