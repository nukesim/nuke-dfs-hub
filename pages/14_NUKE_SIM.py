...
with st.sidebar:
    ...
    field_size=st.number_input("Field size",2,1000000,654761,1,key="field_size")
    entry_fee=st.number_input("Entry fee ($)",.25,10000.,5.,1.,key="entry_fee")
    first_prize=st.number_input("1st prize ($)",1.,10000000.,1000000.,100.,key="first_prize")
    st.caption("Every generated candidate lineup is contest-simmed automatically.")
    contest_iters=st.number_input("Contest iterations",50,5000,contest_iters,50,key="contest_iterations")
    st.divider()
    st.subheader("Portfolio")
    portfolio_size=st.number_input("Portfolio size",1,150,150,1,key="portfolio_size")
    max_overlap=st.slider("Max player overlap",4,8,7,1,key="max_player_overlap")
    path_balance=st.slider("Path diversification",0.,3.,1.25,.25,key="path_diversification")
    max_player_exp=st.slider("Max player exposure %",10,100,45,5,key="max_player_exposure")
    max_qb_exp=st.slider("Max QB exposure %",5,100,30,5,key="max_qb_exposure")
...
pool_game_tab,pool_current_tab=st.tabs(["🏈 Game-by-Game","👥 Current Player Pool"])
with pool_game_tab:
...
if st.button("☢️ RUN NUKE SIM",type="primary",use_container_width=True):
    ...
    with st.status("NUKE SIM is running...",expanded=True) as status:
        ...
        # Build the requested FLEX construction inside the candidate engine. The old
        # FanDuel path generated as many as 5,000 unrestricted lineups and filtered
        # them afterward, which made a 1,450-lineup workspace needlessly take minutes.
