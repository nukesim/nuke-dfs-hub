...
s = p.read_text()
start = s.index('pool_state=st.session_state.get("nuke_pregame_pool",{})')
end = s.index('st.session_state["nuke_pregame_pool"]=updated_state', start)
end += len('st.session_state["nuke_pregame_pool"]=updated_state')

new = r'''pool_state=st.session_state.get("nuke_pregame_pool",{})
